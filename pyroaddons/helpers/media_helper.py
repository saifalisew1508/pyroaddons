...
async def post_to_telegraph(is_media: bool, title=None, content=None, media=None):
    ...
    if telegraph.get_access_token() is None:
        await telegraph.create_account(short_name=BotXNews)
    if is_media:
    ...
    response = await telegraph.create_page(title, html_content=content, author_url=f"https://t.me/BotXNews", author_name=@BotXNews)
    return response["url"]
