...
def command(
    ...
    **kwargs,
    ...
    .. code-block:: python
        ...
        async def start(pyroaddons: Client, message):
    ...
    def wrapper(func):
        async def decorator(pyroaddons: Client, message: pyrogram.types.Message):
            if is_disabled:
            ...
            if self_admin:
                me = await pyroaddons.get_chat_member(message.chat.id, (await pyroaddons.get_me()).id)
                if me.status not in (
            ...
            try:
                await func(pyroaddons, message, *args, **kwargs)
