...
def callback(
    ...
    **kwargs,
    ...
    def wrapper(func):
        async def decorator(pyroaddons: Client, q: pyrogram.types.CallbackQuery):
            if is_bot:
                me = await pyroaddons.get_chat_member(
                    q.message.chat.id, (await pyroaddons.get_me()).id
                )
                if me.status not in (
            ...
            try:
                await func(pyroaddons, q, *args, **kwargs)
