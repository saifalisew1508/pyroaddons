...
from pyroaddons.config import DEVS
...
def adminsOnly(
    ...
    def decorator(func):
        @wraps(func)
        async def wrapper(
            pyroaddons: Client, message: Union[CallbackQuery, Message], *args, **kwargs
        ):
            ...
            if msg.chat.type == ChatType.PRIVATE and not (only_dev or only_owner):
                if allow_pm:
                    return await func(pyroaddons, message, *args, *kwargs)
            ...
            try:
                bot = await chat.get_member(pyroaddons.me.id)
